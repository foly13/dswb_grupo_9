...
router.put('/:id', async (req, res) => {
  try {
    const tasks = await getTasks(); // Leemos las tareas
    const taskIndex = tasks.findIndex(task => task.id === parseInt(req.params.id)); // Buscamos el índice de la tarea
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    const newTasks = tasks.filter(task => task.id !== parseInt(req.params.id)); // Filtramos para eliminar la tarea por ID
